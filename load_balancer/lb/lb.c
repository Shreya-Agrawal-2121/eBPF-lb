// Group members: Shreya Agrawal(20CS10058), Abhijeet Singh(20CS30001)
#include <unistd.h>
#include <stdio.h>
...
struct bpf_map *ipm, *threadm, *queuem;
int queue[100000];
...
int main(int argc, char *argv[])
{
    ...
    printf("Server 1 running at IP %s port 8080\n", argv[1]);
    printf("Server 2 running at IP %s port 8080\n", argv[2]);
    printf("Server 3 running at IP %s port 8080\n", argv[3]);
    ...
    queuem = bpf_object__find_map_by_name(bpfobj, "packet_queue");
    queuemap = bpf_map__fd(queuem);
    if (queuemap < 0)
    {
        printf("Finding map in obj file failed\n");
        exit(EXIT_FAILURE);
    ...
    while (1)
    {
        ...
        {
            int delayed_packet;
            if (!bpf_map_lookup_elem(queuemap, NULL, &delayed_packet))
            {
                bpf_map_lookup_and_delete_elem(queuemap, NULL, &delayed_packet);
                printf("Packet with payload %d being redirected to server %d\n", delayed_packet, serveridx);
                sendto(sockfd, &delayed_packet, sizeof(delayed_packet), 0, (const struct sockaddr *)&recvaddr, sizeof(recvaddr));
            }
        }
    }
    return 0;
