...
int lb_packets(struct xdp_md *ctx){
    ...
    if(is_server || map_empty == 3){
        ...
        int delayed_packet;
        if(!bpf_map_peek_elem(&packet_queue, &delayed_packet)){
            char redirect_msg[] = "A delayed packet will be redirected to server %u\n";
            bpf_trace_printk(redirect_msg, sizeof(redirect_msg), is_server);
            return XDP_PASS;
        }
        else{
            int *f_threads;
            f_threads = bpf_map_lookup_elem(&free_threads, &is_server);
            if(f_threads != NULL){
                *f_threads = *f_threads + 1;
                bpf_map_update_elem(&free_threads, &is_server, f_threads, 2);
            }
            return XDP_DROP;
        }
    ...
    if(!max_f || map_empty == 3){
        ...
        int packet_data = *payloadData;
        bpf_map_push_elem(&packet_queue, &packet_data, 2);
